...
try (FileWriter fw = new FileWriter(dir+date+"/list"+date+".html");FileWriter wf = new FileWriter(dir+"index.html")) {
	...
	//sb.append("<div  style=\"display: flex;flex-direction: row;flex-wrap: wrap;justify-content: space-evenly;\">");
        sb.append("<ol style='margin-left:25%'>");
...
//System.in.read();
	...
	StringBuilder frame = new StringBuilder();
       ...
       // frame.append("<meta name=\"viewport\" content=\"width=device-width,initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no\" />");
        frame.append("<style type=\"text/css\">.item{border: solid 1px;width:480px;max-height:80px;background-color:pink;}</style>");
        frame.append("<style type=\"text/css\">.current{border: solid 1px;width:480px;max-height:80px;background-color:greenyellow;}</style>");
        ...
        frame.append("</head><body style='font-size: smaller;'>");
        ...
        frame.append("<h1 id='control'  style='display:none;color:red;text-align: center;'><a href='javascript:void(0);' onclick='prevNext(-1)'>←←上一首</a>");
	frame.append("&nbsp;&nbsp;|&nbsp;&nbsp;<a href='javascript:void(0);' onclick='continuePlay()'></a>");
	frame.append("&nbsp;&nbsp;|&nbsp;&nbsp;<a href='javascript:void(0);' onclick='prevNext(1)'>下一首→→</a></h1>");
...
System.out.println(listm[i].getName());
       ...
       // frame.append("showStatus();");
        ...
        frame.append("else ");
        frame.append("document.getElementById('showView').style.setProperty('display','block');");
        frame.append("};");
